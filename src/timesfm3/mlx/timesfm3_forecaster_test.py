...
class TimesFM3MlxModelTest(unittest.TestCase):
  ...
  def test_from_hf_config_parses_frozen_running_stats(self):
    cfg = configs.TimesFM3MlxConfig.from_hf_config({"use_frozen_running_stats": True})
    self.assertTrue(cfg.use_frozen_running_stats)
    # The default (absent / False) builds normally.
    cfg = configs.TimesFM3MlxConfig.from_hf_config({"use_frozen_running_stats": False})
    self.assertFalse(cfg.use_frozen_running_stats)
